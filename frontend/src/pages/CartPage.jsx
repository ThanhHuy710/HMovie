...
export default function CartPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Tạo hóa đơn cho giao dịch (backend sẽ tính toán start_date và endDate)
      await api.post("/invoices", {
        user_id: user.profileId,
        plan_id: lastItem.planId,
        totalPrice: parseFloat(((lastItem.plans?.price || 0) * 1000).toFixed(2)), // Chuyển đổi sang VND dạng Decimal
        paymentMethod: "card",
        status: "completed"
      });

      // Cập nhật plan_id cho user
      const updateRes = await api.put(`/users/${user.profileId}`, {
        plan_id: lastItem.planId
      });
      ...
      if (updateRes.data?.result) {
        ...
        const updatedUser = { ...currentUser, planId: lastItem.planId };
